...
try:
    ...
    print("âœ… Blockchain monitor started from main.py")
except Exception as e:
    print(f"âš ï¸ Monitor import error: {e}")
print(f"âœ… Main starting with DB: {'POSTGRES - PERSISTENT' if USE_POSTGRES else 'SQLITE'}")

PORT = int(os.getenv("PORT", 10000))

def run_api():
    import uvicorn
    print(f"ðŸŒ Starting API on 0.0.0.0:{PORT}")
...
if __name__ == "__main__":
    ...
    t.start()
    time.sleep(3)
    print("âœ… API thread started")

    kp = threading.Thread(target=keep_alive_ping, daemon=True)
    kp.start()
    print("âœ… Keep-alive ping started")
    ...
    while True:
        try:
            ...
            print(f"ðŸŒ WEBAPP_URL: {WEBAPP_URL}")
            print(f"ðŸ¤– BOT_USERNAME: @{BOT_USERNAME}")

            async def clear_webhook():
                try:
                    b = Bot(token=BOT_TOKEN)
                    await b.delete_webhook(drop_pending_updates=True)
                    print("âœ… Webhook deleted, pending cleared")
                    # Also getMe to test token
                    me = await b.get_me()
                    print(f"âœ… Bot verified: @{me.username}")
                    await b.close()
                except Exception as e:
            ...
            print(f"ðŸš€ Bot starting polling (retry delay {retry_delay}s)...")
            ...
            print(f"âš ï¸ Conflict detected: Another bot instance is running. Message: {e}")
            print(f"â³ Waiting {retry_delay}s before retry... Render may have 2 instances running.")
            print("ðŸ’¡ FIX: In Render Dashboard -> Set 1 instance only (Scaling -> Manual -> 1), or wait for old instance to die.")
            ...
            if "Conflict" in err_str or "terminated by other getUpdates" in err_str:
                print(f"âš ï¸ Conflict in exception: {e}")
            ...
            if "Flood control exceeded" in err_str:
                ...
                print(f"â³ Flood control: waiting {wait}s")
                time.sleep(wait)
                continue
