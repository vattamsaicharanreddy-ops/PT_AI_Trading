...
"""PT_AI Trading API - Pro Admin with Group Direct Add + Conflict Safe"""
...
def rows_as_dicts(rows): return [dict(row) for row in rows]
def get_tier(balance):
    for index, (minimum, percentage) in enumerate(TIERS):
        if balance >= minimum: return index, minimum, percentage
...
def process_invoice_payment(invoice_id_str, tx_hash, actual_amount):
    ...
    try:
        ...
        if float(actual_amount) < expected - 0.01: return False, f"Amount mismatch expected {expected}"
...
def generate_trades():
    ...
    for i in range(42):
        ...
        if rng.random() > 0.27: pnl = abs(pnl)
...
def tg_api_call(method, payload):
    global BOT_TOKEN
    if not BOT_TOKEN: return {"ok": False, "error": "BOT_TOKEN not set"}
    try:
...
def api_create_invoice(req: InvoiceRequest, tg_id: int = Query(0)):
    ...
    ensure_user(user_id)
    if req.amount < 20: return {"ok": False, "error": "Min 20 USDT"}
    inv = invoice_id()
...
def api_withdraw(req: WithdrawalRequest, tg_id: int = Query(0)):
    ...
    if req.amount < 10: return {"ok": False, "error": "Min 10 USDT"}
    if req.amount > withdrawable: return {"ok": False, "error": "Insufficient wallet"}
    conn = get_conn()
    try:
        cur = cursor(conn)
...
def admin_env_save(req: EnvSave):
    ...
    try:
        with open("bot_token.txt","w") as ft: ft.write(BOT_TOKEN)
    except: pass
    try:
        env_content = f"BOT_TOKEN={BOT_TOKEN}\\nGROUP_ID={GROUP_ID}\\nCHANNEL_ID={CHANNEL_ID}\\nSUPPORT_ID={SUPPORT_ID}\\nBOT_USERNAME={os.getenv('BOT_USERNAME','')}\\n"
        with open(".env","w") as fe: fe.write(env_content)
        if os.path.exists("/data"):
            with open("/data/env_backup.txt","w") as fb: fb.write(env_content)
    except Exception as e: print(f"env save file error {e}")
    test_result = {}
    if BOT_TOKEN: test_result = tg_api_call("getMe", {})
...
def admin_get_group_ids():
    if not BOT_TOKEN: return {"ok": False, "error": "Set BOT_TOKEN first"}
    try:
...
def admin_group_members():
    ...
    try:
        cur=cursor(conn)
        try:
            cur.execute("SELECT * FROM group_members ORDER BY id DESC LIMIT 200")
        except:
            # create table if missing
            if USE_POSTGRES:
                cur.execute("""CREATE TABLE IF NOT EXISTS group_members (id SERIAL PRIMARY KEY, user_id BIGINT, username TEXT, group_name TEXT, group_id TEXT, method TEXT, status TEXT DEFAULT 'added', invite_link TEXT, created_at TEXT DEFAULT NOW())""")
            else:
                cur.execute("""CREATE TABLE IF NOT EXISTS group_members (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, username TEXT, group_name TEXT, group_id TEXT, method TEXT, status TEXT DEFAULT 'added', invite_link TEXT, created_at TEXT)""")
            conn.commit()
            cur.execute("SELECT * FROM group_members ORDER BY id DESC LIMIT 200")
...
def admin_group_invite(req: GroupInvite):
    ...
    if not chat_id: return {"ok": False, "error": "Set GROUP_ID / CHANNEL_ID env with numeric -100... ID and make bot admin"}
...
def admin_group_bulk_add(req: GroupBulkAdd):
    ...
    try:
        cur=cursor(conn)
        try: cur.execute("SELECT 1 FROM group_members LIMIT 1")
        except:
            if USE_POSTGRES:
                cur.execute("""CREATE TABLE IF NOT EXISTS group_members (id SERIAL PRIMARY KEY, user_id BIGINT, username TEXT, group_name TEXT, group_id TEXT, method TEXT, status TEXT DEFAULT 'added', invite_link TEXT, created_at TEXT DEFAULT NOW())""")
            else:
        ...
        def to_chat_id(v):
            ...
            if v.startswith("https://t.me/"):
                uname = v.split("/")[-1].split("?")[0]
                return "@"+uname
            return v
        chat_id_map = {"group": GROUP_ID or GROUP_LINK, "channel": CHANNEL_ID or CHANNEL_LINK, "support": SUPPORT_ID or SUPPORT_LINK}
        target_groups = []
        if req.group=="all":
            for g in ["group","channel","support"]:
                cid = to_chat_id(chat_id_map.get(g,""))
                if cid: target_groups.append((g,cid))
        else:
            cid = to_chat_id(chat_id_map.get(req.group, req.group))
            if cid: target_groups.append((req.group, cid))

        flat_ids=[]
        for item in req.user_ids:
            if not item: continue
            s=str(item).strip()
            parts=[p.strip().lstrip("@") for p in s.replace(","," ").split() if p.strip()]
            flat_ids.extend(parts)
        flat_ids=list(dict.fromkeys(flat_ids))[:500]

        logs=[f"Processing {len(flat_ids)} IDs, groups={target_groups}, BOT_TOKEN={'SET' if BOT_TOKEN else 'MISSING'}"]
        added=0; failed=0
        for raw_str in flat_ids:
            if not raw_str: continue
            uid=None; username=""
            if raw_str.isdigit():
                try: uid=int(raw_str)
                except: uid=None
            else: username=raw_str
            display_uid=uid if uid else raw_str
            try:
                if uid: ensure_user(uid, username or f"user_{uid}")
                elif username:
                    fake_id=abs(hash(username))%900000000+1000000000
                    ensure_user(fake_id, username)
                    uid=fake_id
                    display_uid=f"{username}({fake_id})"
            except Exception as e: logs.append(f"ensure_user fail {raw_str}: {e}")

            for gname,gid in target_groups:
                if not gid:
                    logs.append(f"No chat_id for {gname}")
                    failed+=1
                    continue
                invite_link=""; status="failed"; direct_ok=False
                if uid and BOT_TOKEN:
                    r1=tg_api_call("unbanChatMember", {"chat_id": gid, "user_id": uid, "only_if_banned": False})
                    if r1.get("ok"):
                        direct_ok=True
                        logs.append(f"✅ {display_uid} -> {gname} unban success")
                    else:
                        r_inv=tg_api_call("createChatInviteLink", {"chat_id": gid, "member_limit": 1, "expire_date": int((datetime.utcnow()+timedelta(hours=24)).timestamp())})
                        if r_inv.get("ok"):
                            invite_link=r_inv["result"].get("invite_link","")
                            logs.append(f"🔗 {display_uid} -> {gname} invite: {invite_link}")
                            if isinstance(uid,int) and uid<2000000000:
                                dm=tg_api_call("sendMessage", {"chat_id": uid, "text": f"🚀 You were added to {gname}! Join: {invite_link}"})
                                if dm.get("ok"): logs.append(f"📨 DM sent to {display_uid}")
                                else: logs.append(f"⚠️ DM fail {display_uid}: {dm.get('description')}")
                            direct_ok=True
                        else:
                            logs.append(f"❌ {display_uid}->{gname} fail: {r_inv.get('description')} | {r1.get('description')}")
                if direct_ok:
                    status="added" if not invite_link else "invite_created"
                    added+=1
                else:
                    if not BOT_TOKEN:
                        logs.append(f"⚠️ BOT_TOKEN missing, logging {display_uid}->{gname} as manual")
                        ...
                        status="failed"
                try:
                    cur.execute(f"INSERT INTO group_members (user_id,username,group_name,group_id,method,status,invite_link,created_at) VALUES ({ph()},{ph()},{ph()},{ph()},{ph()},{ph()},{ph()},{ph()})", (int(uid) if isinstance(uid,int) else 0, username, gname, str(gid), req.method, status, invite_link, datetime.utcnow().isoformat()))
                    conn.commit()
                except Exception as e: logs.append(f"DB err {e}")
        return {"ok": True, "added": added, "failed": failed, "logs": logs[-150:]}
    except Exception as e:
        import traceback; traceback.print_exc()
        return {"ok": False, "added": 0, "failed": 1, "logs": [f"EXCEPTION: {e}"]}
    finally: put_conn(conn)
...
@app.get("/health")
def health(): return {"ok":True,"db":"POSTGRES" if USE_POSTGRES else "SQLITE","path":None if USE_POSTGRES else SQLITE_PATH, "group_add_ready": bool(BOT_TOKEN), "bot_token_set": bool(BOT_TOKEN)}
