...
app = FastAPI(title="PT_AI Trading ULTRA V5 WEBHOOK")
...
def webhook_get(token: str):
    return {"ok": True, "message": "Webhook active - POST Telegram updates here"}
...
def env_status():
    return {"has_bot_token": bool(os.getenv("BOT_TOKEN")), "BOT_TOKEN":"SET" if os.getenv("BOT_TOKEN") else "MISSING","GROUP_ID":os.getenv("GROUP_ID","-100...")}
...
@app.get("/health")
def health(): return {"ok":True,"db":"POSTGRES" if USE_POSTGRES else "SQLITE","mode":"WEBHOOK - NO CONFLICT"}

# Alias
@app.get("/api/deposit/invoice_status/{invoice_id}")
def invoice_status_alias2(invoice_id:str):
    conn=get_conn()
    try:
        cur=cursor(conn); cur.execute(f"SELECT * FROM deposits WHERE invoice_id={ph()}",(invoice_id,)); d=cur.fetchone()
        if not d: return {"error":"not found"}
