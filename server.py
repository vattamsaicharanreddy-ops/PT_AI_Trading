...
"""PT_AI Trading API - ULTRA V5 FINAL - Fixed 404 + Conflict"""
...
def create_invoice(user_id:int, req:InvoiceRequest):
    ...
    try:
        ...
        return {"invoice_id":inv,"address":addr,"amount":req.amount,"network":req.network,"expires_at":exp.isoformat(),"qr":f"{addr}","expected_amount":req.amount}
    finally: put_conn(conn)

@app.post("/api/deposit/check_now/{invoice_id}")
def check_now(invoice_id: str):
    conn=get_conn()
    try:
        cur=cursor(conn); cur.execute(f"SELECT * FROM deposits WHERE invoice_id={ph()}",(invoice_id,)); dep=cur.fetchone()
        if not dep: return {"ok":False,"error":"Not found"}
        return {"ok":False,"status":val(dep,"status"),"message":"Auto verifier checks every 15s"}
    finally: put_conn(conn)
...
def binance_trades():
    ...
    rng=random.Random(now.hour*60+now.minute)
    trades=[]
    current = now
    for i in range(num_trades):
        delta_min = random.randint(30,90)
        current = current - timedelta(minutes=delta_min)
        time_str = f"{current.hour:02d}:{current.minute:02d}"
...
def admin_withdraw_action(action: IdAction):
    ...
    try:
        ...
        cur.execute(f"INSERT INTO admin_logs (admin_action,target_id,target_user_id,details) VALUES ('withdraw',{ph()},{ph()},{ph()})",(action.id, val(wd,"user_id"), action.note or action.action))
        conn.commit()
        return {"ok":True}
    finally: put_conn(conn)
...
def admin_user_action(action: AdminAction):
    ...
    try:
        cur=cursor(conn)
        act=action.action
        if act not in {"add_balance","deduct_balance","set_balance","add_withdrawable","deduct_withdrawable","set_withdrawable","add_profit","ban","unban","delete","reset_profit","clear_tasks"}:
...
def bulk_add(payload: GroupBulkAdd):
    return {"ok":True,"logs":[f"Queued {len(payload.user_ids)} users for {payload.group} - requires BOT_TOKEN as admin"],"added":len(payload.user_ids),"failed":0}
...
@app.get("/health")
def health(): return {"ok":True,"db":"POSTGRES" if USE_POSTGRES else "SQLITE","path":None if USE_POSTGRES else SQLITE_PATH,"routes":["/api/me/{id}","/api/tasks/list/{id}","/api/binance/trades"]}

# Alias for old clients
@app.get("/api/deposit/invoice_status/{invoice_id}")
def invoice_status_alias(invoice_id:str):
    conn=get_conn()
    try:
