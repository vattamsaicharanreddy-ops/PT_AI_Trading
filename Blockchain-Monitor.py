...
def get_tron_transactions():
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            data=json.loads(resp.read().decode()); return data.get("data",[])
    except Exception as e:
        print(f"[Monitor] Tron fetch error: {e}"); return []
def get_bsc_transactions():
    try:
        ...
        print(f"[Monitor] BSC error: {e}"); return []
def verify_pending_deposits():
    ...
    try:
        ...
        tron_txs=get_tron_transactions(); bsc_txs=get_bsc_transactions()
        for dep in pending:
            try:
                d=dict(dep); inv=d.get("invoice_id"); user_id=d.get("user_id"); expected=float(d.get("expected_amount",0) or d.get("amount",0) or 0); network=d.get("network","TRC20"); exp_str=d.get("expires_at"); created_str=d.get("created_at")
                try:
                    exp_dt=datetime.fromisoformat(exp_str) if exp_str else datetime.utcnow()
                    if datetime.utcnow()>exp_dt:
                        cur.execute(f"UPDATE deposits SET status='expired' WHERE invoice_id={ph()}",(inv,)); conn.commit(); print(f"[Monitor] Expired {inv}"); continue
                except: pass
                try: created_dt=datetime.fromisoformat(created_str) if created_str else datetime.utcnow()-timedelta(minutes=16)
                except: created_dt=datetime.utcnow()-timedelta(minutes=16)
                matched_tx=None; matched_amount=0
                if network=="TRC20":
                    for tx in tron_txs:
                        try:
                            to_addr=tx.get("to","") or tx.get("to_address",""); value=tx.get("value","0") or tx.get("quant","0"); tx_id=tx.get("transaction_id","") or tx.get("transactionHash",""); timestamp=tx.get("block_timestamp",0) or tx.get("timestamp",0)
                            ...
                            if str(to_addr).lower()==DEPOSIT_ADDR_TRON.lower():
                                ...
                                if tx_time>=created_dt-timedelta(minutes=2):
                                    if abs(amount-expected)<0.5 or amount>=expected-0.01:
                                        ...
                                        if not cur.fetchone():
                                            matched_tx=tx_id; matched_amount=amount; break
                        except: continue
                elif network in ["BEP20","ERC20"]:
                    for tx in bsc_txs:
                        try:
                            to_addr=tx.get("to","").lower(); value=tx.get("value","0"); tx_hash=tx.get("hash",""); time_stamp=int(tx.get("timeStamp","0"))
                            try:
                                amount=float(value)/(10**18)
                                if amount<0.01: amount=float(value)/1_000_000
                            except: amount=0
                            if to_addr==DEPOSIT_ADDR_BSC:
                                ...
                                if tx_time>=created_dt-timedelta(minutes=2):
                                    if abs(amount-expected)<0.5 or amount>=expected-0.01:
                                        ...
                                        if not cur.fetchone():
                                            matched_tx=tx_hash; matched_amount=amount; break
                        except: continue
                if matched_tx:
                    from server import process_invoice_payment
                    ok,msg=process_invoice_payment(inv, matched_tx, matched_amount or expected)
                    if ok: print(f"[Monitor] ✅ AUTO-VERIFIED {inv} {matched_tx} {matched_amount} user {user_id}")
            except Exception as e:
                print(f"[Monitor] Error {e}")
        conn.commit()
    except Exception as e:
        print(f"[Monitor] Loop error {e}"); import traceback; traceback.print_exc()
    finally:
        try:
            from database import put_conn; put_conn(conn)
        except: pass
def blockchain_monitor_loop():
    print("🔍 Blockchain Auto-Verifier Started - 15s interval")
    time.sleep(10)
    while True:
        try: verify_pending_deposits()
        except Exception as e: print(f"[Monitor] Loop exception {e}")
        time.sleep(15)
